...
describe("castValue(value, type)", () => {
  ...
  describeEach("invalid casts", [
    ['irrelevant', 'not valid'],
  ], ([input, badValue]) => {
    it("should throw a TypeError", () => {
      expect((castValue as any)(input, badValue))
    })
      .shouldFail()
      .rescue(reason => {
  ...
  describe("invalid numbers", () => {
    it("should parse as NaN", () => {
      expect(Number.isNaN(castValue('invalid', OptionType.NUMBER) as number)).to.be.true;
    });
  });
});