...
`;
  ...
  const suites = [describe("describe", suite => {
    ...
    return suite.beforeEach(() => {
      ...
      expect$1(typeof subject.it).to.equal("function");
    }).it("should have an `run` method", () => {
      expect$1(typeof subject.run).to.equal("function");
    }).it("should have a `tap` method", () => {
      expect$1(typeof subject.tap).to.equal("function");
    }).describe("Suite#it", suite => suite.it("should enqueue a spec", () => {
      expect$1(subject.specs).to.have.lengthOf(0);
      subject.it("it", () => {});
      expect$1(subject.specs).to.have.lengthOf(1);
